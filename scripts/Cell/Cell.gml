...
/// -----------------------------------------------------------------------
/// Standalone wrapper functions (mirror constructor methods for callsites
/// that prefer function-call style).
/// -----------------------------------------------------------------------

/// @description Check if cell direction is open
function Cell_isOpen(cell, dir, prevDir = -1, size = -1) {
    if ((cell.position.y == CELL_BOUNDARY_ROW_A && cell.position.x == 0 && dir == GRID_DIRECTION.DOWN) ||
        (cell.position.y == CELL_BOUNDARY_ROW_B && cell.position.x == 0 && dir == GRID_DIRECTION.UP)) {
        return false;
    }
    if (size == 2 && (dir == prevDir || ((dir + 2) mod 4) == prevDir)) {
        return false;
    ...
    if (cell.next[dir] != noone && !cell.next[dir].filled) {
        if (!(cell.next[dir].next[GRID_DIRECTION.LEFT] != noone &&
              !cell.next[dir].next[GRID_DIRECTION.LEFT].filled)) {
            return true;
        }
    }
    return false;
...
function Cell_connect(cell, dir) {
    ...
    if (cell.position.x == 0 && dir == GRID_DIRECTION.RIGHT) {
        cell.connections[GRID_DIRECTION.LEFT] = true;
    }
}
...
function Cell_isCrossCenter(cell) {
    return cell.connections[GRID_DIRECTION.RIGHT] &&
           cell.connections[GRID_DIRECTION.UP] &&
           cell.connections[GRID_DIRECTION.DOWN] &&
           cell.connections[GRID_DIRECTION.LEFT];
}
